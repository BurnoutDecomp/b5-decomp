...
# whose pad-pursuit run reached FinishCurrentMode naturally before this fix and then fired ONE
# "Unknown finish line" assert (scratch/bugtest/runs/fxgs2_tailing/20260923_135945, finish 6th).
...
# The race is ENDED by the harness debug finish (-DebugFinishPos 6 at 25 s of mode time: the
# console's own ModeManager::FinishCurrentModeNextUpdateWithFinishPosition, a 6th place -- no win,
# no medal), because the pursuit race is at the mercy of whatever else stops it first (the second
# pursuit run, 20260924_073625, stopped on an unrelated Sound assert before its natural finish).
#   powershell -ExecutionPolicy Bypass -File tools/tests/run_case.ps1 -Case b5-decomp/tests/FxFlowFinishLine.ps1
...
$case.Run.DebugFinishPos = 6
$case.Run.DebugFinishAt = 25
