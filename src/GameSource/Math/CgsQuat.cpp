...
// fdivs) over a 4-float quaternion (lanes x,y,z,w) and a 16-float cMatrix (the four
// 4-float lane rows xa/ya/za/wa). The rodata constants resolve as:
//   flt_82001C98 = 1.0   flt_82001CC0 = 0.0   flt_82001D9C = 2.0
//   flt_82004EF4 = 4.0   flt_82003F40 = 0.25
//
// Matrix float-index -> cMatrix lane map (idx = byte_offset/4):
//   0..3   = xa.{x,y,z,w}   (row 0)
//   4..7   = ya.{x,y,z,w}   (row 1)
//   8..11  = za.{x,y,z,w}   (row 2)
//   12..15 = wa.{x,y,z,w}   (row 3)
...
void cQuat::FromMatrix(const cMatrix& arMatrix)
{
    const f32 lfM00 = arMatrix.xa.x;   // idx0
    const f32 lfM01 = arMatrix.xa.y;   // idx1
    const f32 lfM02 = arMatrix.xa.z;   // idx2
    const f32 lfM10 = arMatrix.ya.x;   // idx4
    const f32 lfM11 = arMatrix.ya.y;   // idx5
    const f32 lfM12 = arMatrix.ya.z;   // idx6
    const f32 lfM20 = arMatrix.za.x;   // idx8
    const f32 lfM21 = arMatrix.za.y;   // idx9
    const f32 lfM22 = arMatrix.za.z;   // idx10
    ...
    if (lfLargest == lfWCandidate)
    {
        ...
        this->x = (lfM21 - lfM12) * lfScale;
        this->y = (lfM02 - lfM20) * lfScale;
        this->z = (lfM10 - lfM01) * lfScale;
        this->w = std::sqrt(lfFour) * 0.25f;
    }
    else if (lfLargest == lfXCandidate)
    {
        const f32 lfFour = lfXCandidate * 4.0f;
        const f32 lfScale = 1.0f / std::sqrt(lfFour);
        this->x = std::sqrt(lfFour) * 0.25f;
        this->y = (lfM10 + lfM01) * lfScale;
        this->z = (lfM20 + lfM02) * lfScale;
        this->w = (lfM21 - lfM12) * lfScale;
    }
    else if (lfLargest == lfYCandidate)
    {
        const f32 lfFour = lfYCandidate * 4.0f;
        const f32 lfScale = 1.0f / std::sqrt(lfFour);
        this->x = (lfM10 + lfM01) * lfScale;
        this->y = std::sqrt(lfFour) * 0.25f;
        this->z = (lfM21 + lfM12) * lfScale;
        this->w = (lfM02 - lfM20) * lfScale;
    }
    else
    {
        const f32 lfFour = lfZCandidate * 4.0f;
        const f32 lfScale = 1.0f / std::sqrt(lfFour);
        this->x = (lfM20 + lfM02) * lfScale;
        this->y = (lfM21 + lfM12) * lfScale;
        this->z = std::sqrt(lfFour) * 0.25f;
        this->w = (lfM10 - lfM01) * lfScale;
    }
}
...
void cQuat::ToMatrix(cMatrix& arMatrix) const
{
    const f32 lfX = this->x;
    const f32 lfY = this->y;
    const f32 lfZ = this->z;
    const f32 lfW = this->w;
    ...
    arMatrix.xa.x = 1.0f - (lf2YY + lf2ZZ);   // idx0  = 1 - 2y^2 - 2z^2
    arMatrix.xa.y = lf2XY - lf2WZ;            // idx1  = 2xy - 2wz
    arMatrix.xa.z = lf2XZ + lf2WY;            // idx2  = 2xz + 2wy
    arMatrix.xa.w = 0.0f;                     // idx3

    arMatrix.ya.x = lf2XY + lf2WZ;            // idx4  = 2xy + 2wz
    arMatrix.ya.y = 1.0f - (lf2XX + lf2ZZ);   // idx5  = 1 - 2x^2 - 2z^2
    arMatrix.ya.z = lf2YZ - lf2WX;            // idx6  = 2yz - 2wx
    arMatrix.ya.w = 0.0f;                     // idx7

    arMatrix.za.x = lf2XZ - lf2WY;            // idx8  = 2xz - 2wy
    arMatrix.za.y = lf2YZ + lf2WX;            // idx9  = 2yz + 2wx
    arMatrix.za.z = 1.0f - (lf2XX + lf2YY);   // idx10 = 1 - 2x^2 - 2y^2
    arMatrix.za.w = 0.0f;                     // idx11

    arMatrix.wa.x = 0.0f;                     // idx12
    arMatrix.wa.y = 0.0f;                     // idx13
    arMatrix.wa.z = 0.0f;                     // idx14
    arMatrix.wa.w = 1.0f;                     // idx15
}
