...
// MomentController::NewMoment's AllocateVoid<MomentNewCarJoined> placement-constructs the moment, which emits its
// vftable, so every slot needs a body. Read off the console vftable off_820074EC (AllocateVoid<MomentNewCarJoined>
// @0x8222EC68 stores it at +0) and the ICF-folded slot bodies it points at:
...
namespace BrnDirector
{
bool MomentNewCarJoined::Prepare(void* /*lrBehaviourController*/)
{
    SetState(E_STATE_INVALID_SEARCHING);   // li r10, 1 ; stw r10, 0x174(r3)
    return true;                           // li r3, 1
}

void MomentNewCarJoined::Destruct()
{
    // 0x8284CB38 is a lone `blr`: nothing to tear down.
}

void MomentNewCarJoined::SetParameters(const Moment::Parameters* lpParameters)
{
