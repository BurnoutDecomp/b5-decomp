...
namespace BrnGui
{
    ...
    struct CrashNavSettings : public CgsGui::State
    {
        ...
        // (CN_D_DETAIL <-TOGGLE-> CN_SETTINGS <-TOGGLE-> ON_PLAY), so an un-hatched
        // placeholder would trap the player in an invisible state with the world paused.
        // Bodies (stand-ins, in BrnScreenStatesDataLinkStubs.cpp) register for the
        // controller event and drain 45/50 -> "GO_BACK", 54/55 -> "TOGGLE_LEFT"/
        // "TOGGLE_RIGHT". DELETE-WHEN the real class TU lands (the virtuals themselves
        // match the DWARF shape and stay).
