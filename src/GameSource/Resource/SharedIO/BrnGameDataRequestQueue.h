...
namespace GameDataIO
{
    ...
    struct RequestInterface
    {
        ...
        // NOT baked: the caller supplies the <=8-char vehicle NAME, the builder compresses it
        // through BrnResource::MakeVehicleId, and the caller picks the asset SET.
        //
        // ⚠ The 4th parameter is a `const char*` NAME, not a prebuilt CgsID. The DWARF says
        // so (:126 and the per-N repeats), and the X360 keeps MakeVehicleId inside the
        // builder, so a CgsID parameter would move a call out into every caller.
        //
        // <4096>'s out-of-line body is an EXPORT HOLE, but the console inlines the whole
        // builder into TrafficEntityModule::LoadData @0x82746A88 twice, in the ATTRIBS pass
        // (stage 8) and the PHYSICS pass (stage 6). Both stage the same 32-byte record and
        // differ only in meType (4 vs 1). Record layout: miEventId +0x00, mpReceiverQueue
        // +0x04, miPoolId +0x08, mId +0x10 (64-bit CgsID), meType +0x18, mbFailFlag +0x1C,
        // posted with AddEvent<LoadGameDataEvent>(..., 26). Body in
