...
#include <algorithm>   // std::sort (models the X360 std::_Sort<PropContact*,int> introsort call)

// BrnPhysics::ContactSpy::ContactSpyQueue<PropContact, 100>::SortAndCreateRunList<100>
//   @ X360 0x825AFDF8   (home: GameSource/Physics/ContactSpies/BrnContactSpyQueue.cpp, DWARF-attested
...
// the queue header deliberately does not pull in) and the <PropContact,100>::<100> specialisation is
// explicitly instantiated at the bottom -- mirroring the committed
...
// (SortAndCreateRunList_PropContact_100_100, BrnPhysicsModuleBridgeFunctions.cpp).
//
// Behaviour (X360 asm):
...
//     and start index seed the new run (asm 0x825B0128-40: vspltisw v0,0 then vmr v12{5,6,7},v0 with
...
//
// The 112-byte contact stride (asm `mulli r11,r11,0x70`, `addi r26,r26,0x70`) matches
// sizeof(PropContact) == 112 (BrnContactSpyEvents.h); the 80-byte run-data stride is committed in
// BrnContactSpyRunList.h.
...
// Explicit instantiation -- the smashable-prop contact pass:
// ContactSpyQueue<PropContact,100>::SortAndCreateRunList<100>(ContactSpyRunList<100>*).
template void
ContactSpyQueue<PropContact, 100>::SortAndCreateRunList<100>(ContactSpyRunList<100>*);
