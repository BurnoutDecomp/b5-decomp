...
#include <algorithm>   // std::sort (models the X360 std::_Sort<TrafficContact*,int> introsort call)

// BrnPhysics::ContactSpy::ContactSpyQueue<TrafficContact, 400>::SortAndCreateRunList<64>
//   @ X360 0x825AF3E8   (home: GameSource/Physics/ContactSpies/BrnContactSpyQueue.cpp, DWARF-attested
...
// the queue header deliberately does not pull in) and the <TrafficContact,400>::<64> specialisation
// is explicitly instantiated at the bottom -- mirroring the committed
// ContactSpyQueue_PropContact_100_SortAndCreateRunList_100.cpp /
// ContactSpyQueue_PhysicalCarPartContact_150_SortAndCreateRunList_50.cpp /
...
// (BrnPhysicsModuleBridgeFunctions.cpp).
//
// Behaviour (X360 asm):
...
//     and start index seed the new run (asm 0x825AF720-38: vspltisw v0,0 then vmr v12{5,6,7},v0 with
...
//
// The 96-byte contact stride (asm `slwi r10,r11,1; add r11,r11,r10; slwi r11,r11,5` == len*3*32, and
// `addi r26,r26,0x60`) matches sizeof(TrafficContact) == 96 (BrnContactSpyEvents.h); the 80-byte
// run-data stride is committed in BrnContactSpyRunList.h.
...
// Explicit instantiation -- the traffic-vehicle contact pass:
// ContactSpyQueue<TrafficContact,400>::SortAndCreateRunList<64>(ContactSpyRunList<64>*).
template void
ContactSpyQueue<TrafficContact, 400>::SortAndCreateRunList<64>(ContactSpyRunList<64>*);
