...
#include <algorithm>   // std::sort (models the X360 std::_Sort<RaceCarContact*,int> introsort call)

// BrnPhysics::ContactSpy::ContactSpyQueue<RaceCarContact, 300>::SortAndCreateRunList<8>
//   @ X360 0x825AEED8   (home: GameSource/Physics/ContactSpies/BrnContactSpyQueue.cpp, DWARF-attested
...
// the queue header deliberately does not pull in) and the <RaceCarContact,300>::<8> specialisation is
...
//     and start index seed the new run (asm 0x825AF210-28: vspltisw v0,0 then vmr v12{5,6,7},v0 with
...
//
// The 96-byte contact stride (asm `slwi ..,5` == liIndex*3*32, `addi r26,r26,0x60`) matches
// sizeof(RaceCarContact) == 96 (BrnContactSpyEvents.h); the 80-byte run-data stride is committed in
...
// Explicit instantiation -- the race-car contact pass:
// ContactSpyQueue<RaceCarContact,300>::SortAndCreateRunList<8>(ContactSpyRunList<8>*).
template void
ContactSpyQueue<RaceCarContact, 300>::SortAndCreateRunList<8>(ContactSpyRunList<8>*);
