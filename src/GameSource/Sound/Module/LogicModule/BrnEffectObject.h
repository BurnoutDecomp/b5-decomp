...
// FLAG: minimal — the owning CgsEffectBase home is not yet reconstructed; this
// is the shape needed to declare BrnEffectObject's RTTI hooks.
template <typename T>
...
// entry point this TU calls (RemoveRequests) is declared here; the full type is
// reconstructed in its own home (GameSource/Sound/BrnResourceRegistrar.{h,cpp}).
// FLAG: minimal forward-declared surface — RemoveRequests is resolved at link
// time against the BrnResourceRegistrar TU; the body is NOT defined here.
...
// BrnEffectObject implements as its second base (sub-object vptr @ this+4 on
// X360). The detach/registrar paths route through this base, matching the
// observed `this-4` / `this+4` thunk adjustments.
struct IResourceRequester
{
