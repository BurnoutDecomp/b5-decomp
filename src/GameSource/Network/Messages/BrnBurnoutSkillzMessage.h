...
// The immediate base is ReliableMessage: PackOrUnpack calls
// ReliableMessage::PackOrUnpack directly (TestConnectionMessage has its own, separate
// PackOrUnpack copy, which is not the one called). GetPackedMessageSize tail-calls the
// shared body that ReliableMessage / TestConnectionMessage::GetPackedMessageSize fold
// onto, so it chains ReliableMessage::GetPackedMessageSize.
//
...
{
    struct BurnoutSkillzMessage : CgsNetwork::ReliableMessage
    {
