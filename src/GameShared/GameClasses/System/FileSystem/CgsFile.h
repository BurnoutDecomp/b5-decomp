...
namespace CgsFileSystem
{
    ...
    // ---- DWARF CgsFileRW.h (forward-declared `File`) ----
    // The concrete file handle the FileSystem owns 16 of (maFiles): the BaseFile base plus the
    // raw rw::core::filesys backing state and the async open/read/write/close machinery. Its
    // FULL definition is homed in CgsFileRW.h (the DWARF home). Forward-declared here so pointer/
    // reference consumers of CgsFile.h see the name; any TU that needs the complete type (the
    // FileSystem, which embeds maFiles by value) includes CgsFileRW.h.
    class File;
