...
// The per-VFT object-size table (byte_82144A18[AptVFT_NumVFTs]): storage in
// AptGlobals.cpp; contents regenerated by StaticInitialize -- see the note below.
//
...
// item-to-item. Storage lives in AptGlobals.cpp (zero-init); the CONTENTS are
// class-set-generated on both platforms -- the X360 .rdata bytes are RECOVERED
// (0x82144A18 dump; shipped scan result min 0x00 / max 0x44) and the XB1 twin
// (sub_14082D9F0) scans its own x64-size table byte_140C13B48 -- so
// StaticInitialize (AptValueGCPoolManager.cpp) regenerates the x64 contents by
// the same rule, sizeof() over the reconstructed class set, before its scan
// (retiring AptInit.cpp's old 4/256 min/max override FLAG).
// ---------------------------------------------------------------------------
