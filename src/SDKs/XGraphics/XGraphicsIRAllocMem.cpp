#include "SDKs/XGraphics/XGraphicsIRAllocMem.h"
...
// ===========================================================================
// XGRAPHICS::IRAllocMem -- bodies reconstructed from BURNOUT_X360_ARTIST.XEX.
// The PowerPC asm is authoritative for every store, immediate and branch. No
// reference source and no DWARF exist for this TU; see XGraphicsIRAllocMem.h for
// the shape notes. Each store / branch below is store-for-store with the X360
// asm, mirroring the committed IR-instruction sibling XGraphicsIRAllocPos.
// ===========================================================================

namespace XGRAPHICS
{

// @ 0x82C2B7E0 -- return the instruction-type tag string "alloc_mem"
// (lis/addi aAllocMem; blr).
const char* IRAllocMem::InstType()
{
    return "alloc_mem";
}

// @ 0x82C2B7F0 -- the scalar deleting destructor is a compiler-generated thunk
...
IRAllocMem::~IRAllocMem()
{
}

// Construction inlined into NewInst on X360 (no standalone ctor symbol). The asm
// calls IRInst::IRInst(this, 0x8F, context) -- the base ctor + implicit IRInst
// vtable stamp -- then, on entry to this derived ctor body, the IRAllocMem vtable
// (off_821B2690) is stamped, and the three flag stores below run:
//   this->muUnkA8      = 0;               // stw 0, 0xA8(r3)
//   this->miNumInputs  = 0;               // stw 0, 0x14(r3)
//   this->miNumOutputs = 1;               // stw 1, 0x10(r3)
//   *this              = &off_821B2690;   // stw off_821B2690, 0(r3)  (implicit)
IRAllocMem::IRAllocMem(CFG* apContext)
    : IRInst(143) // li r4, 0x8F -- IRAllocMem hardcodes its own opcode
{
...
// @ 0x82C2C488 -- carve a prefixed IRAllocMem block from the context's arena and
...
IRAllocMem* IRAllocMem::NewInst(s32 aiOpcode, CFG* apContext)
{
    // The X360 factory ABI passes (opcode, context) uniformly across node kinds,
    // but IRAllocMem hardcodes its own opcode 0x8F (asm: li r4, 0x8F), so the
    // passed opcode is dead -- r3 is overwritten before it is read.
    (void)aiOpcode;

    void* lpMem = ArenaAllocPrefixed(apContext->mpArena, sizeof(IRAllocMem));
    if (!lpMem)
        return nullptr;

    return new (lpMem) IRAllocMem(apContext);
}
